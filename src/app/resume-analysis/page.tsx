...
export default function ResumeAnalysisPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const res = await fetch(`/api/users/save-analysis?userId=${userId}`);

        // Check if response is JSON
        const contentType = res.headers.get("content-type");
        if (!contentType || !contentType.includes("application/json")) {
          const text = await res.text(); // get the HTML/text for debugging
          throw new Error(`Unexpected response format:\n${text}`);
        }
        const analyses = await res.json();
        console.log(analyses)
  
 const improvements = analyses.analyses[0]?.resume_data?.answer?.improvements;
console.log(improvements)
if (improvements && improvements.length > 0) {
    improvements.forEach((imp:string, index:string) => {
        console.log(`Recommendation ${index + 1}: ${imp.current}`);
    });
} else {
    console.log("No recommendations available");
        ...
        if (!res.ok) {
          throw new Error(analyses.error || "Failed to fetch data");
        }

        setAnalyses(analyses.analyses);
      } catch (err: any) {
...
//     <div>
//       <p>Error: {error}</p>
//       <p>Check the network request in DevTools for more details.</p>
//     </div>
//   );

//   return (
//     <div>
//       <h1>Resume Analyses</h1>
//       {analyses.length === 0 ? (
...
//         <ul>
//   {analyses.length > 0 && (
//     <li key={analyses[0].analysis_id}>
//       <pre>{JSON.stringify(analyses[0].analysis_id)}</pre>
//     </li>
//   )}
// </ul>
//      )}
